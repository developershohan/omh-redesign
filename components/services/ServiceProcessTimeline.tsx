          ...
          <ol className="relative col-span-8 border-l border-line pl-9 max-sm:pl-7">
            {steps.map(([stepTitle, stepBody], index) => (
              <li key={stepTitle} className="group/step relative pb-10 last:pb-0">
                <span
                  aria-hidden
                  className="absolute -left-[51px] top-0.5 flex size-[30px] items-center justify-center rounded-full border border-line bg-white font-sans text-[13px] font-semibold tabular-nums text-amber-deep transition-colors group-hover/step:border-teal group-hover/step:bg-teal group-hover/step:text-ink max-sm:-left-[43px]"
                >
                  {index + 1}
                </span>
