      ...
      <div className="container-omh pb-[clamp(56px,38px+2.9vw,88px)] pt-[clamp(48px,34px+2.3vw,80px)]">
        <div className="grid grid-cols-12 items-center gap-x-12 gap-y-12 max-lg:block">
          <Reveal className="col-span-7 max-lg:mb-12">
            ...
            <h1 className="mb-6 mt-7 max-w-[20ch] font-sans text-h1 font-semibold text-balance">Local SEO services that help <span className="text-[#3d709f]">nearby customers find your business</span></h1>
          ...
          <Reveal className="col-span-5">
            <Pointer><div className="pointer-parallax"><MediaFrame kind="screen" theme="seo" ratio="16/11" title="Local search visibility view" note="Replace with an approved local rank, Google Business Profile or location-performance screen with dates and metric definitions visible." /></div></Pointer>
            <dl className="mt-6 grid grid-cols-2 gap-4 border-t border-line pt-5 text-[13px]"><div><dt className="text-muted">Published package range</dt><dd className="mt-1 font-semibold">£450–£700</dd></div><div><dt className="text-muted">Published contract term</dt><dd className="mt-1 font-semibold">3 months</dd></div></dl>
          </Reveal>
        </div>
      </div>
    </section>
      ...
      <div className="container-omh section-md">
        <Reveal>
          <div className="grid grid-cols-12 items-end gap-x-12 gap-y-9 max-lg:block">
            <div className="col-span-5 max-lg:mb-9"><p className="text-[12px] font-semibold uppercase tracking-[0.17em] text-[#446f9f]">Local search footprint</p><h2 className="mt-5 max-w-[15ch] font-sans text-h2 font-semibold">Your profile, website and business details should tell the same story.</h2><p className="mt-6 max-w-[48ch] text-[17px] leading-relaxed text-[#10243a]/68">We review the surfaces customers use before calling, requesting directions or visiting your website.</p></div>
            <MediaFrame kind="screen" theme="seo" ratio="16/10" title="Google Business Profile and local performance" note="Use an approved profile or reporting screen showing the business, date range and meaningful actions." className="col-span-7" />
          </div>
          <div className="mt-7 grid grid-cols-12 gap-5"><MediaFrame kind="image" theme="seo" ratio="5/4" title="Business details and directory review" note="Add an approved NAP and citation audit view." className="col-span-5 max-md:col-span-12" /><MediaFrame kind="video" theme="seo" ratio="16/8" title="Local visibility review" note="Replace with a short profile, website and location-page walkthrough." className="col-span-7 max-md:col-span-12" /></div>
        </Reveal>
      </div>
    </section>
  );
    ...
    <SharedServiceFitSection
      ...
      intro={
        <div className="flex items-end justify-between gap-10 max-lg:block">
          <div>
            <p className="text-[12px] font-semibold uppercase tracking-[0.15em] text-[#3d709f]">A useful starting point</p>
          ...
          <p className="max-w-[48ch] text-[17px] leading-relaxed text-ink/70 max-lg:mt-5">This service is designed for businesses serving real customers in defined locations. It is not a route to invented locations or guaranteed rankings.</p>
        </div>
      }
...
export function LocalSeoWorkstreams() {
  return (
    <Band label="Local SEO coverage" tone="mist">
      <Reveal>
        <div className="flex items-end justify-between gap-10 max-lg:block"><h2 className="max-w-[18ch] font-sans text-h2 font-semibold">What local SEO work can include.</h2><p className="max-w-[50ch] text-[17px] leading-relaxed text-[#10243a]/68 max-lg:mt-5">The selected package and written proposal should confirm which profile, website, directory, review and reporting tasks are included.</p></div>
        <div className="mt-12 grid grid-cols-4 gap-5 max-xl:grid-cols-2 max-md:grid-cols-1">{content.workstreams.map((stream, index) => <article key={stream.title} className="surface-card rounded-card border border-[#c4d4e5] bg-white p-7"><span className="text-[11px] font-semibold tracking-[0.14em] text-[#d56d47]">0{index + 1}</span><p className="mt-4 text-[11.5px] font-semibold uppercase tracking-[0.15em] text-[#3d709f]">{stream.label}</p><h3 className="mt-3 font-sans text-h4 font-semibold">{stream.title}</h3><p className="mt-3 text-[16px] leading-relaxed text-[#10243a]/68">{stream.body}</p><ul className="mt-5 border-t border-[#d5e0eb] pt-3">{stream.items.map((item) => <li key={item} className="flex gap-3 py-2 text-[14px] leading-relaxed"><Check className="mt-0.5 size-4 shrink-0 text-[#ee8c67]" />{item}</li>)}</ul></article>)}</div>
      </Reveal>
    </Band>
  );
}
    ...
    <ServiceProcessTimeline
      ...
      intro={
        <>
          ...
          <p className="mt-5 max-w-[42ch] text-[17px] leading-relaxed text-ink/70">We agree the locations, access and measurement first, then work through the profile, website and local business data in priority order.</p>
        </>
      }
...
export function LocalSeoFinalCTA() {
  return (
    <section className="bg-[#76a9e8] text-[#0e2035]"><div className="container-omh section-md grid grid-cols-12 items-center gap-x-12 gap-y-8 max-lg:block"><Reveal className="col-span-8"><p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-[#0e2035]/55">Local visibility starting point</p><h2 className="mt-5 max-w-[20ch] font-sans text-h2 font-semibold">Check where your local visibility needs work.</h2><p className="mt-5 max-w-[62ch] text-lead leading-relaxed text-[#0e2035]/72">Tell us the services and locations that matter, share your website and business profile, and explain which calls, visits or enquiries you want to improve.</p></Reveal><Reveal className="col-span-4 max-lg:mt-8"><div className="flex flex-col gap-3"><Button href="/contact" variant="inverse" arrow data-event="local_seo_final_cta_click">Check My Local Visibility</Button><Button href="/contact" variant="secondary" data-event="local_seo_brief_start">Send a Local SEO Brief</Button></div><p className="mt-5 text-[14px] text-[#0e2035]/62"><Link href={`mailto:${company.email}`} className="underline underline-offset-4">{company.email}</Link></p></Reveal></div></section>
  );
}
