...
function SocialDashboard() {
  return (
    <div className="service-media-frame relative overflow-hidden rounded-card border border-[#b8cde3] bg-[#10243a] p-5 text-oninverse shadow-xl" role="img" aria-label="Illustrated social media campaign planning dashboard">
      <div className="flex items-center justify-between border-b border-oninverse/10 pb-4"><span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#9bc3f3]">Campaign studio</span><span className="flex gap-1.5"><i className="size-2 rounded-full bg-[#ef8067]" /><i className="size-2 rounded-full bg-[#76a9e8]" /><i className="size-2 rounded-full bg-oninverse/30" /></span></div>
      <div className="mt-5 grid grid-cols-[1.2fr_.8fr] gap-4 max-sm:grid-cols-1">
        <div className="rounded-xl bg-surface p-5 text-ink"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-deep">Content calendar</p><div className="mt-4 grid grid-cols-4 gap-2">{Array.from({ length: 12 }).map((_, i) => <span key={i} className={`aspect-square rounded-md ${[1,4,6,9,10].includes(i) ? "bg-[#ef8067]" : i === 7 ? "bg-[#76a9e8]" : "bg-tint-blue"}`} />)}</div><div className="mt-5 h-2 rounded-full bg-tint-blue"><span className="block h-full w-[72%] rounded-full bg-[#d79a37]" /></div></div>
        <div className="grid gap-4"><div className="rounded-xl border border-oninverse/12 bg-oninverse/[0.045] p-4"><p className="text-[10px] uppercase tracking-[0.13em] text-oninverse/50">Channels</p><p className="mt-2 font-sans text-[26px] font-semibold">4</p></div><div className="rounded-xl border border-oninverse/12 bg-oninverse/[0.045] p-4"><p className="text-[10px] uppercase tracking-[0.13em] text-oninverse/50">Status</p><p className="mt-2 text-[18px] font-semibold text-[#a8dfd8]">Ready for review</p></div></div>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[10px] font-semibold uppercase tracking-[0.1em] text-oninverse/55"><span className="rounded-lg border border-oninverse/10 py-3">Create</span><span className="rounded-lg border border-oninverse/10 py-3">Approve</span><span className="rounded-lg border border-oninverse/10 py-3">Report</span></div>
    </div>
  );
}
...
export function SocialGoals() {
  return <Band label="Define the goal" tone="navy"><Reveal><div className="grid grid-cols-12 gap-x-12 gap-y-10 max-lg:block"><div className="col-span-5 max-lg:mb-10"><p className="text-[14px] font-semibold uppercase tracking-[0.15em] text-[#9bc3f3]">Need help?</p><h2 className="mt-5 max-w-[14ch] font-sans text-h2 font-semibold">Start with the outcome, not the posting schedule.</h2><p className="mt-5 max-w-[48ch] text-body leading-relaxed text-oninverse/68">Social media marketing can include text, images, video, community activity and paid advertising. A clear strategy keeps those activities connected to the business goal.</p></div><ol className="col-span-7 grid grid-cols-2 gap-4 max-md:grid-cols-1">{content.goals.map((goal,index)=><li key={goal} className="rounded-card border border-oninverse/12 bg-oninverse/[0.04] p-6"><span className="text-[11px] font-semibold text-[#ef8067]">0{index+1}</span><p className="mt-4 text-body leading-relaxed text-oninverse/82">{goal}</p></li>)}</ol></div><div className="mt-10 flex flex-wrap items-center justify-between gap-5 border-t border-oninverse/12 pt-6"><p className="max-w-[70ch] text-[14px] leading-relaxed text-oninverse/62">Platform audience figures move constantly, so we work from current platform documentation at planning stage rather than quoting a number here. [CONFIRM SERVICE DETAIL]</p><Button href="/contact" variant="ghost-white" small>Talk Through Your Goals</Button></div></Reveal></Band>;
}

export function SocialSupport() {
  return <Band label="How we can help" tone="mist"><Reveal><div className="flex items-end justify-between gap-10 max-lg:block"><h2 className="max-w-[17ch] font-sans text-h2 font-semibold">Creative, optimisation and account support in one joined-up service.</h2><p className="max-w-[51ch] text-body leading-relaxed text-ink/68 max-lg:mt-5">From branded assets and profile design through to account refreshes and review management.</p></div><div className="mt-11 grid grid-cols-3 gap-5 max-lg:grid-cols-2 max-md:grid-cols-1">{content.support.map((item,index)=><article key={item.title} className="surface-card rounded-card border border-[#bfd1e3] bg-surface p-7"><span className="text-[11px] font-semibold text-amber-deep">0{index+1}</span><h3 className="mt-4 font-sans text-h4 font-semibold">{item.title}</h3><p className="mt-3 text-body leading-relaxed text-ink/70">{item.body}</p></article>)}</div></Reveal></Band>;
}
...
export function SocialFAQ() {
  return (
    <ServiceFaqSection
      ...
      descriptionClassName="max-w-[42ch] text-body text-ink/68"
      sticky={false}
    />
  );
}
