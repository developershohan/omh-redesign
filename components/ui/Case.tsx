...
export function CaseStudyFeature({ c, media }: { c: CaseMeta; media: React.ReactNode }) {
  ...
  return (
    <div className="grid grid-cols-[7fr_5fr] items-start gap-8 max-lg:grid-cols-1">
      {media}
      <div className="rounded-card border border-line bg-white p-8 max-sm:p-6">
...
export function CaseStudyRow({ c }: { c: CaseMeta }) {
  return (
    <Link
      href={c.href}
      className="grid grid-cols-[2fr_5fr_3fr_auto] items-center gap-5 border-b border-line px-1 py-5 transition-colors hover:bg-white/75 max-sm:grid-cols-1 max-sm:gap-1.5"
    >
