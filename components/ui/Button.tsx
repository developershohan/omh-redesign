...
const styles: Record<Variant, string> = {
  primary: "border-transparent bg-teal text-white hover:bg-teal-dark hover:-translate-y-px",
  secondary: "border-ink text-ink hover:bg-ink/5",
  inverse: "border-transparent bg-white text-ink hover:bg-soft hover:-translate-y-px",
...
export function Button({
  ...
  return (
    <Link
      href={href}
      {...props}
      className={`group inline-flex items-center gap-2 rounded-button border-[1.5px] font-semibold leading-none transition
        ${small ? "px-4.5 py-3 text-[15px]" : "px-6 py-4 text-base"} ${styles[variant]} ${props.className ?? ""}`}
    >
      {children}
...
export function TextLink({
  ...
  return (
    <Link
      href={href}
      {...props}
      className={`group inline-flex items-center gap-2 font-semibold text-teal hover:underline underline-offset-4 ${props.className ?? ""}`}
    >
      {children}
